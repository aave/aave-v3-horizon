...
import {IPoolConfigurator} from 'src/contracts/protocol/pool/PoolConfigurator.sol';

contract PoolConfiguratorInitReservesRwaTest is TestnetProcedures {
  ...
  function test_initReserves_validNumberOfAssets(TestVars[128] memory t, uint8 length) public {
    ...
    for (uint256 i = 0; i < length; i++) {
      input[i] = _generateRwaInitReserveInput(t[i], report, poolAdmin, true);
  ...
  function test_reverts_initReserves_maxAssets(TestVars memory t, uint8 lengthSeed) public {
    ...
    ConfiguratorInputTypes.InitReserveInput[]
      memory input = new ConfiguratorInputTypes.InitReserveInput[](length);
    for (uint256 i = 0; i < length; i++)
      input[i] = _generateRwaInitReserveInput(t, report, poolAdmin, true);
