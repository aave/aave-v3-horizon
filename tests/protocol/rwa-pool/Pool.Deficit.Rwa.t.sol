...
contract PoolDeficitRwaTests is TestnetProcedures {
  ...
  function test_eliminateReserveDeficit_exactDeficit(
    ...
    _mintATokens(tokenList.usdx, coverageAdmin, currentDeficit + 1, false);
  ...
  function test_eliminateReserveDeficit_fullUserBalance(
    ...
    _mintATokens(tokenList.usdx, coverageAdmin, currentDeficit / 2, false);
  ...
  function test_eliminateReserveDeficit_surplus(
    ...
    _mintATokens(tokenList.usdx, coverageAdmin, currentDeficit + 1000, false);
  ...
  function test_eliminateReserveDeficit_parcial(
    ...
    _mintATokens(tokenList.usdx, coverageAdmin, currentDeficit, false);
  ...
  function test_interestRate() external {
    ...
    vm.prank(poolAdmin);
    contracts.poolAddressesProvider.setAddress(bytes32('UMBRELLA'), coverageAdmin);
    _mintATokens(tokenList.usdx, bob, 1_000_000 ether, false);
    vm.prank(bob);
    ...
    _mintATokens(tokenList.usdx, coverageAdmin, deficit, false);
    vm.startPrank(coverageAdmin);
  ...
  function _createReserveDeficit(
    ...
    _mintATokens(tokenList.buidl, alice, 27_000e6, true);

    if (mintBorrowableAssets) {
      // setup available amount to borrow
      _mintATokens(borrowAsset, carol, borrowAmount, false);
    }
    ...
    // authorize liquidator to hold buidl collateral
    vm.prank(poolAdmin);
    TestnetRWAERC20(tokenList.buidl).authorize(bob, true);

    vm.prank(bob);
    IERC20(borrowAsset).approve(address(contracts.poolProxy), borrowAmount);
    vm.prank(bob);
    contracts.poolProxy.liquidationCall(tokenList.buidl, borrowAsset, alice, borrowAmount, false);
  ...
  function _mintATokens(address underlying, address receiver, uint256 amount, bool isRwa) internal {
    if (isRwa) {
      vm.startPrank(poolAdmin);
      TestnetRWAERC20(underlying).authorize(receiver, true);
      TestnetRWAERC20(underlying).mint(receiver, amount);
      vm.stopPrank();
    } else {
    ...
    contracts.poolProxy.supply(underlying, amount, receiver, 0);
    vm.stopPrank();
  }
